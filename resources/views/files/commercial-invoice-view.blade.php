...
<table>
	...
	<tr>
		...
		<td class="border-black">{{$p->sales_pro_ctn}}</td>
			<td class="border-black">{{$p->sales_total_carton}}</td>
		<td class="border-black">{{$p->sales_pro_price*$p->sales_pro_ctn}}  <?php $t+=$p->sales_pro_price*$p->sales_pro_ctn ?></td>
		<td class="border-black">{{$p->sales_pro_price*$p->sales_pro_ctn*$p->sales_total_carton}} <?php $f+=$p->sales_pro_price*$p->sales_pro_ctn*$p->sales_total_carton ?></td>
...
$rettxt .= " and ";
if($dataecnum < 20){
$rettxt .= $ones[$dataecnum];
}elseif($dataecnum < 100){
$rettxt .= $tens[substr($dataecnum,0,1)];
$rettxt .= " ".$ones[substr($dataecnum,1,1)];
}
}
return $rettxt;
}
