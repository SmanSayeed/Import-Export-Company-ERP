...
@foreach($p as $d)
        ...
        <a href= "/commercial-invoice-edit/{{$d->id}}" class="btn btn-primary">Edit</a>
        @if($d->publish_key == 1)
        <a href="{{route('commercial-invoice-unpublish',$d->id)}}" class="btn btn-danger">Add to CI</a>
        @elseif($d->publish_key == 0)
        <a href="{{route('commercial-invoice-publish',$d->id)}}" class="btn btn-success">Remove From CI</a>
        @endif
        </td>
             </tr>
@endforeach
